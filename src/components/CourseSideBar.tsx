...
const CourseSideBar = ({ course, currentChapterId }: Props) => {
  return (
    <aside
      className={cn(
        "bg-secondary w-full p-5 dark:bg-zinc-900",
        "lg:w-72 xl:w-80 2xl:w-[21rem]",
        "h-full overflow-y-auto",
        "rounded-md shadow-md",
      )}
    >
      <h1 className="text-foreground mb-4 text-2xl font-bold break-words">
        {course.name}
      </h1>
      ...
      <div className="space-y-6">
        {course.units.map((unit, unitIndex) => (
          <div key={unit.id}>
            ...
            <div className="text-foreground text-xl font-bold break-words">
              {unit.name}
            </div>
            ...
            {unitIndex < course.units.length - 1 && (
              <Separator className="bg-border dark:bg-border my-4" />
            )}
          </div>
        ))}
      </div>
